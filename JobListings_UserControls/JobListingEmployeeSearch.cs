...
{
    public partial class JobListingEmployeeSearch : UserControl
    {
        public JobListingEmployeeSearch()
        {
            InitializeComponent();
        }
        SqlConnection con;
        ...
        private void JobListingEmployeeSearch_Load(object sender, EventArgs e)
        {
        ...
        {
            con.Open();
            cmd = new SqlCommand("insert into tbl_applicantation (app_companyemail,app_applicantemail,app_applieddate,app_jobid) values (@CompanyEmail,@ApplicantEmail,@AppliedDate,@JobID)", con);
            cmd.Parameters.AddWithValue("CompanyEmail",lbl_JobVendorEmail.Text);
            cmd.Parameters.AddWithValue("ApplicantEmail", sign.getemail());
            cmd.Parameters.AddWithValue("AppliedDate", DateTime.Today.ToString());
            ...
            {
                MessageBox.Show("Unable To Apply For Job","Error",MessageBoxButtons.OK,MessageBoxIcon.Error);
            }
            con.Close();
            CheckApplicationStatus();
        }
